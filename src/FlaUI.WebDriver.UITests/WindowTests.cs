...
namespace FlaUI.WebDriver.UITests
{
    ...
    public class WindowTests
    {
        ...
        public void SwitchWindow_Default_SwitchesToWindow()
        {
            var driverOptions = FlaUIDriverOptions.TestApp();
            using var driver = new RemoteWebDriver(WebDriverFixture.WebDriverUrl, driverOptions);
            var initialWindowHandles = driver.WindowHandles;
            OpenAnotherWindow(driver);
            var newWindowHandle = driver.WindowHandles.Except(initialWindowHandles).Single();
        ...
        private static void OpenAnotherWindow(RemoteWebDriver driver)
        {
            ...
            driver.FindElement(ExtendedBy.NonCssName("Open _Window 1")).Click();
        }
    }
}
